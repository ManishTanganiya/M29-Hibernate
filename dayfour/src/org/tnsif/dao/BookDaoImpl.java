...
public class BookDaoImpl implements BookDao {
	//object creation to access all CRUD methods
	...
	public List<Book> getAuthorBooks(String author) {
		String query="SELECT COUNT(book.id) FROM BOOK book WHERE book.author=:pAuthor";
	...
	public List<Book> getBookByTitle(String title) {
		String query="SELECT COUNT(book.id) FROM BOOK book WHERE book.title=:pTitle";
		TypedQuery<Book>t1=em.createQuery(query,Book.class);
	...
	public List<Book> getBookByPrice(double low, double high) {
		String query="SELECT COUNT(book.id) FROM BOOK book WHERE book.price between=:low and :high";
		TypedQuery<Book>t1=em.createQuery(query,Book.class);
